...
int main() {
    ...
    std::map<std::string, std::string> fenToBestMove = {
        ...
        {"8/8/8/8/p1k5/P1p4p/2K4P/8 w - - 0 61", "c2c1"} // thought for an eternity, couldnt crack it
    ...
    for (const auto& [fen, bestMove] : fenToBestMove) {
        ...
        Move b = searcher.search(50000, 0, 1); // 1 move to go means use all the time on the clock, current is ten seconds each
